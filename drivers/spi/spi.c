				  ...
				  struct spi_board_info *chip)
{
	...
	if (chip->swnode) {
		...
		if (status) {
			dev_err(NULL, "failed to add software node to '%s': %d\n",
		...
		struct spi_message *msg, bool was_busy)
{
	...
	if (!was_busy && ctlr->auto_runtime_pm) {
		...
		if (ret < 0) {
			...
			dev_err(NULL, "Failed to power device: %d\n",
				ret);
	...
	if (ctlr->prepare_message) {
		...
		if (ret) {
			dev_err(NULL, "failed to prepare message: %d\n",
				ret);
			msg->status = ret;
			spi_finalize_current_message(ctlr);
			return ret;
...
static int spi_init_queue(struct spi_controller *ctlr)
{
	...
	if (IS_ERR(ctlr->kworker)) {
		dev_err(NULL, "failed to create message pump kworker\n");
...
void spi_finalize_current_message(struct spi_controller *ctlr)
{
	...
	if (mesg->prepared && ctlr->unprepare_message) {
		...
		if (ret) {
			dev_err(NULL, "failed to unprepare message: %d\n",
				ret);
		}
	}
...
static int spi_destroy_queue(struct spi_controller *ctlr)
{
	 ...
	 */
	if (ret) {
		dev_err(NULL, "problem destroying queue\n");
		return ret;
	}
...
static int spi_controller_initialize_queue(struct spi_controller *ctlr)
{
	...
	if (ret) {
		dev_err(NULL, "problem initializing queue\n");
		goto err_init_queue;
	}
	ctlr->queued = true;
	ret = spi_start_queue(ctlr);
	if (ret) {
		dev_err(NULL, "problem starting queue\n");
			   ...
			   struct device_node *nc)
{
	...
	if (spi_controller_is_slave(ctlr)) {
		if (!of_node_name_eq(nc, "slave")) {
			dev_err(NULL, "%pOF is not called 'slave'\n",
				nc);
			return -EINVAL;
		}
		return 0;
	...
	if (rc) {
		dev_err(NULL, "%pOF has no valid 'reg' property (%d)\n",
			nc, rc);
		return rc;
	}
...
of_register_spi_device(struct spi_controller *ctlr, struct device_node *nc)
{
	...
	if (!spi) {
		dev_err(NULL, "spi_device alloc error for %pOF\n", nc);
		rc = -ENOMEM;
		goto err_out;
	}
	...
	rc = of_alias_from_compatible(nc, spi->modalias,
				      sizeof(spi->modalias));
	if (rc < 0) {
		dev_err(NULL, "cannot find modalias for %pOF\n", nc);
		goto err_out;
	}
	...
	if (rc) {
		dev_err(NULL, "spi_device register error %pOF\n", nc);
					    ...
					    struct acpi_device *adev)
{
	...
	if (spi_add_device(spi)) {
		...
		dev_err(NULL, "failed to add SPI device %s from ACPI\n",
			dev_name(&adev->dev));
		spi_dev_put(spi);
	}
...
void spi_unregister_controller(struct spi_controller *ctlr)
{
	...
	if (ctlr->queued) {
		if (spi_destroy_queue(ctlr))
			dev_err(NULL, "queue remove failed\n");
	}
	mutex_lock(&board_lock);
	list_del(&ctlr->list);
	mutex_unlock(&board_lock);
...
int spi_controller_suspend(struct spi_controller *ctlr)
{
	...
	if (ctlr->queued) {
		ret = spi_stop_queue(ctlr);
		if (ret)
			dev_err(NULL, "queue stop failed\n");
	}
...
int spi_controller_resume(struct spi_controller *ctlr)
{
	...
	if (ctlr->queued) {
		ret = spi_start_queue(ctlr);
		if (ret)
			dev_err(NULL, "queue restart failed\n");
	}
	return ret;
}
...
static void __spi_transfer_message_noqueue(struct spi_controller *ctlr, struct spi_message *msg)
{
	...
	ret = __spi_pump_transfer_message(ctlr, msg, was_busy);
	if (ret)
		dev_err(NULL, "noqueue transfer failed\n");
	ctlr->cur_msg = NULL;
	ctlr->fallback = false;
